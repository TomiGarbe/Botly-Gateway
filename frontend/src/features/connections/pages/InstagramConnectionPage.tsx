...
export function InstagramConnectionPage() {
  ...
  return <section className="new-connection-page instagram-onboarding">
    ...
    {!connectionId ? <div className="connection-choice-form"><Field label="Nombre de la conexión"><Input value={name} onChange={(event) => setName(event.target.value)} maxLength={160} disabled={isSubmitting} /></Field><button type="button" className="client-button-primary" disabled={isSubmitting} onClick={() => void createInstagramConnection()}>{isSubmitting ? <><LoaderCircle className="animate-spin" size={16} /> Preparando configuración…</> : <><Instagram size={16} /> Configurar Instagram</>}</button>{error ? <p className="client-form-error" role="alert">{error}</p> : null}</div> : <div className="instagram-oauth-card"><CheckCircle2 size={28} aria-hidden="true" /><div><h3>Configuración preparada</h3><p>Autorizá Meta para iniciar las pruebas. El canal no se habilitará hasta superarlas.</p></div><button type="button" className="client-button-primary" onClick={() => window.location.assign(authorizeUrl(connectionId))}>Continuar con Meta</button></div>}
  </section>
}
